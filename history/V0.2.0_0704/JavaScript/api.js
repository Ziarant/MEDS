...
async function rejectFile(file) {
    if (confirm(`确定要拒绝录入文件 "${file.name}" 吗？`)) {
        // 发送拒绝请求
        fetch('http://A012050-1.naton.cn:8081/rejectFile', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function addTestMethod(testMethod, testMethodFiles, technicalFiles, reportModelFiles, competitorDataFiles) {
    ...
    formData.append('testMethodFiles', testMethodFiles);
    formData.append('technicalFiles', technicalFiles);
    formData.append('reportModelFiles', reportModelFiles);
    formData.append('competitorDataFiles', competitorDataFiles);
