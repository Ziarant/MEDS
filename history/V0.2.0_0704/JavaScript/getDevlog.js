    ...
    log.forEach(logItem => {
        ...
        <div class="px-6 py-4 cursor-pointer flex items-center justify-between hover:bg-gray-50 transition-colors duration-150"
            ...
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm"><i class="fa fa-calendar mr-1"></i>${developText}: ${logItem.data}</p>
                </div>
            </div>
            <div class="flex items-center">
                <span
        ...
        if (logItem.nextPlan !== undefined && logItem.nextPlan.length > 0) {
            ...
            logItem.nextPlan.forEach(npItem => {
                ...
                row.innerHTML = `
                <span
                    class="px-2 py-0.5 text-xs font-medium bg-purple-100 text-purple-800 rounded-full mr-2 mt-0.5">未来计划</span>
                <span>${npItem}</span>
                `
                logTable.appendChild(row)
            });
