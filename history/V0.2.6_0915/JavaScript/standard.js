...
const pdfAiChat = document.getElementById('pdf-aiChat')
const pdfFullScreen = document.getElementById('pdf-fullScreeen')
...
function renderStandards(currentData) {
    ...
    currentData.forEach(standard => {
        ...
        if (standard.replace !== undefined && standard.replace !== "") {
            replaceDiv = `<div><i class="fa fa-sync text-primary mr-1"></i> 替代标准: ${standard.replace}</div>`;
        }
                ...
                <div class="flex justify-between items-center text-sm mt-auto pt-3 border-t border-gray-200">
                    <div class="flex space-x-2 justfy-center">
                        ...
                        <span class="text-gray-500 space-x-2">
                            <i class="fa fa-calendar mr-1"></i>${standard.issueDate}
                            <i class="fa fa-calendar-check mr-1"></i>${standard.implementDate}
                        </span>
                    </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-4 w-full">
                ...
                <div class="flex items-center gap-6 md:gap-10 whitespace-nowrap">
                    <div class="text-gray-500 text-sm">
                        <div><i class="fa fa-calendar mr-1"></i>${standard.issueDate}</div>
                        <div><i class="fa fa-calendar-check mr-1"></i>${standard.implementDate}</div>
                    </div>
...
function showModal(standard, typeColor, typeIcon, type, enforceColor, enforceability, adopted, stateColor, state, replaceDiv, adoptedDiv) {
    ...
    let scope = ''
    if (standard.scope !== undefined) {
        scope = standard.scope.replace(/\n/g, '<br>');
    ...
    <div class="mb-6">
        ...
        <div class="flex flex-wrap gap-4 text-sm text-gray-600">
            <div><i class="fa fa-calendar text-primary mr-1"></i> 发布日期: ${standard.issueDate}</div>
            <div><i class="fa fa-calendar-check text-primary mr-1"></i> 实施日期: ${standard.implementDate}</div>
...
pdfAiChat.addEventListener('click', () => {
    let blobUrl = pdfEmbed.src
    fetch(blobUrl)
...
// AI对话：
document.getElementById('pdf-aiChat').addEventListener('click' , () => {
    deepseekBtn.click()
})