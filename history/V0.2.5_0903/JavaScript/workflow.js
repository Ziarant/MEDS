...
const memberFilter = document.querySelectorAll('input[name="member-filter"]');
    ...
    if (currentData.length !== 0) {
        // 处理数据：
         const groupedData = currentData.reduce((acc, item) => {
            ...
            // 过滤无描述的项目：
            if(item.describe[`${currentWeek}`] === '' || item.describe[`${currentWeek}`] === undefined) {
                return acc
            }
...
function filterWorkFlow() {
    ...
    currentData = sortedData.filter(work => {
        ...
        // 状态筛选
        const statusFilter = statusSelect.value ? work.state === statusSelect.value : true;
        // 工作类型筛选
