...
function initTestList(testList) {
    ...
    if (testList) {
        ...
        parsedTest.forEach((test, index) => {
            ...
            testItem.innerHTML = `
                <div class="${pending} bg-white shadow-md rounded-lg p-3 hover:shadow-lg transition-shadow duration-300">
                    ...
                    <p class="text-gray-500 px-2 mb-2">测试标准：<span class="${standardColor}">${test.testStandard}</span></p>
                    <p class="text-gray-700 px-2 mb-2">${test.description}</p>
                    <div>
                        相关文件：
...
function openFileModal(id, type) {
    ...
    documentNames.forEach(fileName => {
        const filePath = 'testMethods/' + `${path}/${type}/` + fileName
