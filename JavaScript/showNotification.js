...
if (typeof addDataButton === 'undefined') {
    addDataButton = document.getElementById('add-data-button')
}

// deepseek
let fileContainer = document.getElementById("files-container")
...
document.getElementById("guide-a").addEventListener("click", function () {
    ...
    const target = '_blank'
    // 创建新页面:
    if (location.includes('selection.html')) {
        console.log('当前已在selection页面')
        window.open("../guide.html", target);
        return
    } else if (location.includes('guide.html')) {
        window.open("../client/guide.html", target);
        return
    }
    // 定向到guide页面的对应锚点
    const locationHash = location.pop().split('.')[0]
    window.open(`../client/guide.html#${locationHash}`, target);
});
...
function yuanbaoWindow() {
    ...
    if (yuanbaoFrame === null) {
        ...
        const overlay = document.createElement('div');
        yuanbaoFrame.className = 'fixed bg-gray-100 top-1/2 left-1/2 border border-gray-600 transform -translate-x-1/2 -translate-y-1/2 bg-white p-3 rounded shadow-lg z-50 w-[95%] h-[95%]';
        overlay.className = `fixed inset-0 bg-white bg-opacity-75 z-40 transition-opacity duration-300`
        ...
        yuanbaoFrame.innerHTML = `
            <div class="flex items-center justify-between space-x-4 mt-1 ml-3 mr-3">
                ...
                <button id="close-yuanbaoFrame" class="text-gray-400 hover:text-gray-600 transition-colors" title="最小化">
    ...
    if (chatWindow === null) {
        ...
        chatWindow.innerHTML = `
            ...
            <div class="flex-1 flex flex-col">
                <div class="flex text-center items-center justify-between p-3 bg-gray-100 border-b border-gray-600">
                    <div class="flex text-center space-x-2 items-center justify-between">
                        ...
                        <button id="ai-clear-button" class="bg-blue-500 text-white rounded-lg py-2 px-4 hover:bg-blue-600 transition-colors" title="清空本对话">
                    ...
                    <div>
                        ...
                        <p class="text-xs">内容由 AI 生成，无法确保真实准确，仅供参考</p>
                    </div>
                ...
                <div id="files-container" class="flex flex-wrap items-start bg-gray-100 justify-start">
                </div>
                ...
                <div class="flex items-center justify-between space-x-4">
                    ...
                    <div>
                        ...
                        <button id="chatWindow-addFile-btn" class="bg-gray-500 text-white rounded-lg py-2 px-4 hover:bg-gray-600 transition-colors"
                            title="添加文件 | 使用数据库后端进行解析，当前支持.pdf文件">
        ...
        document.getElementById('chatWindow-addFile-btn').addEventListener('click', () => {
            ...
            input.addEventListener('change', (event) => {
                ...
                fileContainer = document.getElementById("files-container")
                for (let i = 0; i < files.length; i++) {
                    const file = files[i];

                    // 创建容器
                    const fileContentContainer = document.createElement('div');
                    fileContentContainer.className = 'message-container flex bg-blue-200 border border-gray-300 text-black rounded-lg py-2 px-2 md-1 w-[33%] break-words';
                    fileContentContainer.title = file.name
                    let { fileIcon, fileIconBg, fileIconColor } = getFileIconbyFileType(file.type)
                    fileContentContainer.innerHTML = `
                    <div class="min-w-10 min-h-10 rounded-lg ${fileIconBg} flex items-center justify-center mr-2">
                        <i class="fa ${fileIcon} ${fileIconBg} text-2xl ${fileIconColor}"></i>
                    </div>
                    <div>
                        <p class="line-clamp-1">${file.name}</p>
                    ...
                    // 发送pdf至后台解析
                    data = parseUploadPDF(file)
                    .then((data) => {
                        const pdfContent = document.createElement('textarea')
                        pdfContent.classList = 'hidden pdfcontent'
                        pdfContent.id = `${file.name}`
                        pdfContent.value = `[**文件**]${file.name}${data.content}`;
                        fileContentContainer.querySelector('.file-parse-status').textContent = '解析完成';
                        fileContentContainer.appendChild(pdfContent)
                    })
...
function toggleChatWindow() {
    ...
    if(!userConfig){
        showNotification('请先设置apikey:\n个人中心-账户设置-AI设置', 'warning')
    }
    ...
    document.getElementById('ai-clear-button').onclick = function () {
        currentMessages = []
        document.getElementById('chat-messages').innerHTML = `
        <div class="items-center text-center text-sm mb-2">
            <p>——————历史对话已清空——————</p>
        </div>
        <div class="flex justify-start mb-4">
            <div class="message-container bg-gray-200 text-black rounded-lg py-2 px-4 max-w-[75%] break-words">
    ...
    document.getElementById('send-chat').onclick = function () {
        ...
        // 检查文件内容：
        if(fileContainer){
            pdfContents = fileContainer.querySelectorAll('.pdfcontent')
            if(pdfContents) {
                pdfContents.forEach(pdfContent => {
                    const fileBox = pdfContent.parentNode
                    if (fileContainer.contains(fileBox)) {
                        fileContainer.removeChild(fileBox);
                        // 修改式样
                        fileBox.classList.replace('bg-blue-200', 'bg-green-200')
                        // 添加到对话框：
                        fileLinkCrad = document.createElement('div')
                        fileLinkCrad.className = 'flex justify-end mb-4';
                        fileLinkCrad.appendChild(fileBox)
                        chatMessages.appendChild(fileLinkCrad);
                    }
                    // console.log(fileBox)
...
function updateMessageContent(id, message, role, model = '') {
    ...
    if (document.getElementById(id) !== null) {
        ...
        if (role === 'user') {
            ...
            if(content.startsWith('<p>[<strong>文件</strong>]')) {
                // 文件消息
                messageDiv.className = 'flex justify-end mb-4';
                // 提取文件名：
                const regex = /\[<strong>文件<\/strong>\]\[\d+\](.*?\.pdf)/;
                ...
                messageDiv.innerHTML = `
                    <div class="message-container flex bg-green-200 border border-gray-300 text-black rounded-lg py-2 px-2 md-1 max-w-[60%] break-words">
                        <div class="min-w-10 min-h-10 rounded-lg ${fileIconBg} flex items-center justify-center mr-2">
                            <i class="fa ${fileIcon} ${fileIconBg} text-2xl ${fileIconColor}"></i>
                        </div>
                        <div>
                            <p class="line-clamp-1">${fileName}</p>
                            <span class="file-parse-status text-gray-400">解析完成</span>
                        </div>
                        <textarea class="hidden">${content}
                        </textarea>
                    </div>
                `;
            } else {
                // 文本消息
                messageDiv.innerHTML = `
                    <div class="message-container bubble bg-green-200 text-black rounded-lg py-2 px-4 max-w-[90%] break-words" title="模型：${model}"></div>
                `;
                messageDiv.querySelector('.message-container').innerHTML = content;
            }
            ...
            messageDiv.innerHTML = `
                <div class="flex flex-col max-w-[90%]">
                    <div class="message-container bubble bg-gray-200 text-black rounded-lg py-2 px-4 max-w-[100%] break-words" title="角色：${role}"></div>
                    <div class="flex justify-end space-x-2">
                        <button class="message-copy-btn block bg-gray-300 border border-gray-400 px-2 py-1 rounded hover:bg-blue-200 mt-1" onclick="copyMessageContent(this)">
                            <i class="fa fa-copy mr-1"></i>复制为html格式
                        </button>
                        <button class="message-copy-md-btn block bg-gray-300 border border-gray-400 px-2 py-1 rounded hover:bg-blue-200 mt-1" onclick="copyMessageContent(this)">
                            <i class="fa fa-copy mr-1"></i>复制为md格式
                        </button>
                        <button class="message-delete-btn block bg-red-300 border border-red-400 px-2 py-1 rounded hover:bg-blue-200 mt-1" onclick="copyMessageContent(this)">
            ...
            `;
            messageDiv.querySelector('.message-container').innerHTML = content;
            messageDiv.querySelector('.message-copy-btn').addEventListener('click', (event) => {
                ...
                try {
                    ...
                    showNotification('html格式已复制到剪切板', success)
                } catch (err) {
                    confirm(cite);
                } finally {
                    // 清除选中状态
            ...
            messageDiv.querySelector('.message-copy-md-btn').addEventListener('click', (event) => {
                const textArea = document.createElement('textarea');
                ...
                try {
                    const success = document.execCommand('copy');
                    showNotification('markdown格式已复制到剪切板', success)
                } catch (err) {
                    confirm(cite);
                } finally {
                    // 清除选中状态
            ...
            messageDiv.querySelector('.message-delete-btn').addEventListener('click', (event) => {
                const messageContainer = messageDiv.querySelector('.message-container');
                if (messageContainer) {
                    messageContainer.remove();
                }
                messageDiv.remove();
                const index = currentMessages.findIndex((message) => message.content === content && message.role === 'assistant')
                if(index) {
                    const confirmResult = confirm(`删除后，聊天记录不可恢复，可能影响AI后续对话的上下文理解`);
                    if(!confirmResult) {return} else {
...
function htmlToMarkdown(html) {
    ...
    function processNode(node) {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            return escapeSpecialChars(node.textContent);
        }
        ...
        switch (tagName) {
            ...
            case 'img':
                ...
                const imgTitle = node.getAttribute('title') ? `"${node.getAttribute('title')}"` : '';
                const imgTitlePart = imgTitle ? ` ${imgTitle}` : '';
            ...
            case 'pre':
                ...
                const language = codeNode ? codeNode.getAttribute('class')?.replace('language-', '') : '';
            ...
            case 'table':
                let tableResult = '';
                const headers = Array.from(node.querySelectorAll('th')).map(th => processNode(th));
                const rows = Array.from(node.querySelectorAll('tr')).slice(1).map(tr => 
                    Array.from(tr.querySelectorAll('td')).map(td => processNode(td))
                );
                
                // 表头
                tableResult += `| ${headers.join(' | ')} |\n`;
                // 分隔线
                tableResult += `| ${headers.map(() => '---').join(' | ')} |\n`;
                // 内容行
                rows.forEach(row => {
                    tableResult += `| ${row.join(' | ')} |\n`;
                });
