...
function initTestList(testList) {
    ...
    if (testList) {
        ...
        parsedTest.forEach((test, index) => {
            ...
            const methodButtonTextColor = testMethodFiles.length > 0 ? 'text-primary' : 'text-gray-400';
            const technicalButtonTextColor = technicalFiles.length > 0 ? 'text-primary' : 'text-gray-400';
            const reportButtonTextColor = reportModelFiles.length > 0 ? 'text-primary' : 'text-gray-400';
            const competitorButtonTextColor = competitorDataFiles.length > 0 ? 'text-primary' : 'text-gray-400';
            ...
            testItem.innerHTML = `
                <div class="${pending} bg-white shadow-md rounded-lg p-3 hover:shadow-lg transition-shadow duration-300 flex flex-col h-full">
                    ...
                    <div class="mt-auto pt-3 border-t border-gray-200">
                        相关文件：
                        <button id="${testItem.id}-method" class="${methodButtonTextColor} hover:underline ${strikethroughtestMethodFiles}">测试方法文件</button>
                        <button id="${testItem.id}-technical" class="${technicalButtonTextColor} hover:underline ${strikethroughtTechnicalFiles}">技术要求文件</button>
                        <button id="${testItem.id}-report" class="${reportButtonTextColor} hover:underline ${strikethroughtReportModelFiles}">报告模板文件</button>
                        <button id="${testItem.id}-competitor" class="${competitorButtonTextColor} hover:underline ${strikethroughtCompetitorDataFiles}">竞品参考数据</button>
                    </div>
                </div>
            `;
