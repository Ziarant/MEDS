...
async function getFile(filename) {
    const response = await fetch(`${serveURL}:${servePost}/getFile`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function downloadFile(file) {
    ...
    const filename = file.name;
    const response = await fetch(`${serveURL}:${servePost}/download`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function downloadByPath(filePath, fileName) {
    showNotification('文件下载已开始', 'info');
    const response = await fetch(`${serveURL}:${servePost}/downloadByPath`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function uploadTestData(testData) {
    const response = await fetch(`${serveURL}:${servePost}/uploadTestData`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function deleteTestData(testData) {
    const response = await fetch(`${serveURL}:${servePost}/deleteTestData`, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json'
        },
...
function checkData(testData) {
    // 发送确认请求
    fetch(`${serveURL}:${servePost}/checkData`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function rejectTestData(testData) {
    // 发送确认请求
    fetch(`${serveURL}:${servePost}/rejectData`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function addTestMethod(testMethod, testMethodFiles, technicalFiles, reportModelFiles, competitorDataFiles) {
    ...
    const repose = await fetch(`${serveURL}:${servePost}/addTestMethod`, {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function checkTestMethod(testMethod) {
    if (confirm(`确定要录入该测试项目吗？`)) {
        const repose = await fetch(`${serveURL}:${servePost}/checkTestMethod`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function rejectTestMethod(testMethod) {
    if (confirm(`确定要拒绝录入该测试项目吗？`)) {
        const repose = await fetch(`${serveURL}:${servePost}/rejectTestMethod`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function getUserList(username) {
    const response = await fetch(`${serveURL}:${servePost}/getUserList`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function resetUserPassword(username, salt, hash_password) {
    ...
    const response = await fetch(`${serveURL}:${servePost}/resetPassword`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function getWorkList(username) {
    const response = await fetch(`${serveURL}:${servePost}/getWorkList`, {
        method: 'POST',
        body: JSON.stringify({
            username
        })
...
async function uploadWorkItem(workItem) {
    const response = await fetch(`${serveURL}:${servePost}/uploadWorkItem`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function getStandardList(username) {
    const response = await fetch(`${serveURL}:${servePost}/getStandardList`, {
        method: 'POST',
        body: JSON.stringify({
            username
        })
...
async function submitStandardForm(formData) {
    ...
    const response = await fetch(`${serveURL}:${servePost}/submitStandard`, 
        {
            method : 'POST',
            body: formData
        }
    ...
    if (response.ok) {
        showNotification('标准添加成功', 'success')
    } else {
        showNotification('标准添加失败', 'warning')
        return
    };
}
...
async function agencySearch(queryURI, website) {
    const response = await fetch(`${serveURL}:${servePost}/search?q=${queryURI}&w=${website}`,
        {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
...
// 获取产品列表：
async function getProductList(username) {
    const response = await fetch(`${serveURL}:${servePost}/getProductList`, {
        method: 'POST',
        body: JSON.stringify({
            username
        })
...
async function submitProduct(formData) {
    const response = await fetch(`${serveURL}:${servePost}/submitProduct`,
        {
            method: 'POST',
            body: formData
        }
    ...
    if (response.ok) {
        showNotification('产品信息添加成功', 'success');
        updateProducts();
    } else {
        showNotification('产品信息添加失败', 'warning')
        return
    };
}
...
async function getLiteratureList(username) {
    const response = await fetch(`${serveURL}:${servePost}/getLiteratureList`, {
        method: 'POST',
        body: JSON.stringify({
            username
        })
...
// 添加清单：
async function submitLiterature(formData) {
    const response = await fetch(`${serveURL}:${servePost}/submitLiterature`,
        {
            method: 'POST',
            body: formData
        }
    ...
    showNotification('文献添加成功', 'success')
    if (response.ok) {
        showNotification('文献添加成功', 'success')
    } else {
...
async function getLiteratureFile(filename) {
    const response = await fetch(`${serveURL}:${servePost}/getLiteratureFile`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    const parse  = await fetch(`${serveURL}:${servePost}/parsePDF`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function uploadDSFile(apiKey, file) {
    ...
    const key = 'sk-gwvfrlfluaeblmorvggjyuftofuizvafkrwavdmlxabyxhwf'
...
// 后端解析文件：'/parseUploadPDF'
async function parseUploadPDF(file) {
    formData = new FormData();
    formData.append('file', file);
    const response = await fetch(`${serveURL}:${servePost}/parseUploadPDF`, {
        method: 'POST',
        body: formData
    })
